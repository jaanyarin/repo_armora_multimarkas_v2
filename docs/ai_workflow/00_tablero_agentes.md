...
| F2-016 | Gestion Zonas y Rutas: migraciones, API, UI CRUD y poligono | lista | `armora-architect` | `armora-product-owner`, `armora-ui-ux`, `armora-database`, `armora-backend-quarkus`, `armora-frontend-web`, `armora-security`, `armora-qa`, `armora-sdd-manager` | F2-015 | V7 crea zonas_rutas; V8 crea zonas_rutas_poligonos; V9 normaliza a zonas + rutas + zonas_poligonos; V10/V11 seeds (40 zonas, 227 rutas); API /api/v1/zonas-rutas con CRUD completo (zonas, rutas, poligonos, estados, eliminacion); admin web 3 paginas (CRUD tabs, mapa poligono canvas, habilitar ventas); builds backend/frontend pasan; ADR-003 documenta normalizacion | `docs/ai_workflow/F2-016_gestion_zonas_rutas.md` |
...
| F2-018 | Integrar Headroom para Codex y OpenCode | documentada | `armora-architect` | `armora-devops`, `armora-security`, `armora-qa`, `armora-sdd-manager` | F2-003 | `opencode.json` registra MCP local `headroom`; plantilla Codex TOML y guia operativa documentan instalacion/uso sin secretos | `docs/ai_workflow/F2-018_headroom_codex_opencode.md` |
