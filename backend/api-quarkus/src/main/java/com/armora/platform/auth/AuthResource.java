...
public class AuthResource {
    ...
    public ResponseWrapper<LoginResponse> login(@Valid LoginRequest request) {
        try (Connection conn = dataSource.getConnection();
                PreparedStatement ps = conn.prepareStatement(
                        "SELECT id, usuario, correo, clave_hash, tipo, estado FROM usuarios WHERE correo = ? OR usuario = ?")) {
            ps.setString(1, request.identificador());
            ps.setString(2, request.identificador());
            try (ResultSet rs = ps.executeQuery()) {
                ...
                return ResponseWrapper.ok(new LoginResponse(pair, tipo, correo));
            }
    ...
    public ResponseWrapper<LoginResponse> refresh(@Valid RefreshRequest request) {
        try {
            ...
            return ResponseWrapper.ok(new LoginResponse(pair, "DESCONOCIDO", ""));
        } catch (WebApplicationException e) {
            throw e;
        } catch (Exception e) {
            throw new WebApplicationException("Error interno", Response.Status.INTERNAL_SERVER_ERROR);
    ...
    public ResponseWrapper<LogoutResponse> logout(@Valid RefreshRequest request) {
        refreshTokenService.invalidate(request.refreshToken());
        return ResponseWrapper.ok(new LogoutResponse("Sesion cerrada"));
    }
    ...
    public ResponseWrapper<MeResponse> me() {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                return ResponseWrapper.ok(new MeResponse(
                        rs.getString("id"),
